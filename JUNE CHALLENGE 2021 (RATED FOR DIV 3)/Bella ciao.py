...
13
Explanation
Test Case 1:On the first day, the rate of production is 1 dollar per day so 1 dollar is printed on the first day.
...
Test Case 2:For the first two days, the rate of production is 1 dollar per day so 1⋅2=2 dollars are printed on the first two days.
...
Test Case 3:For the first two days, the rate of production is 1 dollar per day so 1⋅2=2 dollars are printed on the first two days.
...
for _ in range(int(input())):
    ...
    n = D//d
    if n%2 != 0:
        dollar = d*(n*(p+((n-1)//2)*q))
        dollar += (D%d)*(p+n*q)
    else:
        dollar = d*(n//2)*(2*p+(n-1)*q)
        dollar += (D%d)*(p+n*q)
