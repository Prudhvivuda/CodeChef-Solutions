...
def solve(a, n, b, m):
    ...
    dest = []
    for i in range(n):
        if i == 0 :dest.append(0)
        elif a[i] !=0 :dest.append(0)
        else :dest.append(mx)
    for i in range(n):
        if a[i] == 1 :high = i
        if high != -1 and a[i] == 0 :dest[i] = min(dest[i], i-high)
    for i in range(n-1,-1,-1):
        if a[i] == 2 :low = i
        if low != -1 and a[i] == 0 :dest[i] = min(dest[i], low-i)
    for i in range(m):
        st = b[i] - 1
        if dest[st] != mx :print(dest[st], end=' ')  
        else :print(-1, end=' ')
    print()
